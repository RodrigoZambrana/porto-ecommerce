...
function wc_add_to_cart_message($products, $show_qty = false, $return = false)
{
    ...
    if ('yes' === get_option('woocommerce_cart_redirect_after_add')) {
        ...
        $message = sprintf(
            '<a href="%s" tabindex="1" class="button wc-forward%s">%s</a> %s',
            esc_url($return_to),
    ...
    if (has_filter('wc_add_to_cart_message')) {
        wc_deprecated_function(
            'The wc_add_to_cart_message filter',
            '3.0',
    ...
    if ($return) {
        ...
        wc_add_notice(
            $message,
            apply_filters('woocommerce_add_to_cart_notice_type', 'success')
        );
    }
}
...
function wc_cart_totals_taxes_total_html()
{
    echo apply_filters(
        ...
        wc_price(WC()->cart->get_taxes_total())
...
function wc_cart_totals_coupon_label($coupon, $echo = true)
{
    ...
    $label = apply_filters(
        ...
        sprintf(esc_html__('Coupon: %s', 'woocommerce'), $coupon->get_code()),
        $coupon
    );
...
function wc_cart_totals_coupon_html($coupon)
{
    ...
    $discount_amount_html = apply_filters(
    ...
    echo wp_kses(
        ...
        array_replace_recursive(wp_kses_allowed_html('post'), [
            'a' => ['data-coupon' => true],
        ])
...
function wc_cart_totals_order_total_html()
{
    ...
    if (wc_tax_enabled() && WC()->cart->display_prices_including_tax()) {
        ...
        if (get_option('woocommerce_tax_total_display') === 'itemized') {
            ...
            $tax_string_array[] = sprintf(
                '%s %s',
                wc_price(WC()->cart->get_taxes_total(true, true)),
...
{
    $cart_totals_fee_html = WC()->cart->display_prices_including_tax()
        ? wc_price($fee->total + $fee->tax)
        : wc_price($fee->total);
...
{
    $chosen_methods = WC()->session->get('chosen_shipping_methods');
    $chosen_method = isset($chosen_methods[$key])
        ? $chosen_methods[$key]
        : false;
    ...
    if (
        !$chosen_method ||
        $changed ||
        !isset($package['rates'][$chosen_method]) ||
        count($package['rates']) !== $method_count
    ) {
        $chosen_method = wc_get_default_shipping_method_for_package(
            $key,
            $package,
            $chosen_method
        );
 ...
 * @return string
 */
function wc_get_default_shipping_method_for_package(
    $key,
    $package,
    $chosen_method
) {
    ...
    $is_local_pickup_chosen = in_array(
        $chosen_method_id,
        $local_pickup_method_ids,
        true
    );
...
function wc_shipping_methods_have_changed($key, $package)
{
    ...
    $previous_shipping_methods = WC()->session->get(
        'previous_shipping_methods'
    );
    // Get new and old rates.
    $new_rates = array_keys($package['rates']);
    $prev_rates = isset($previous_shipping_methods[$key])
        ? $previous_shipping_methods[$key]
        : false;
    // Update session.
