...
<!-- WISHLIST TABLE -->
<table class="shop_table cart wishlist_table wishlist_view traditional responsive<?php echo !$enable_drag_n_drop
    ? ''
    : ' sortable',
    function_exists('yith_wcwl_is_mobile') && yith_wcwl_is_mobile()
        ? ' mobile'
        : ''; ?> " data-pagination="<?php echo esc_attr(
     $pagination
 ); ?>" data-per-page="<?php echo absint(
    $per_page
); ?>" data-page="<?php echo absint(
    $current_page
	...
	<thead class="<?php echo !$wishlist || !$wishlist->has_items()
     ? 'd-none'
     : ''; ?>">
	<tr>
		<?php if ($show_cb):
      ...
      $column_count++; ?>
         ...
         'yith_wcwl_wishlist_view_quantity_heading',
         __('Cantidad', 'yith-woocommerce-wishlist')
     ); ?>
				</span>
			</th>
		<?php
      ...
      $column_count++; ?>
			<th class="product-arrange">
				<span class="nobr">
					<?php echo esc_html(
         apply_filters(
             'yith_wcwl_wishlist_view_arrange_heading',
             __('Arrange', 'yith-woocommerce-wishlist')
         )
     ); ?>
				</span>
			</th>
 ...
 if ($wishlist && $wishlist->has_items()):
     foreach ($wishlist_items as $item):
         ...
         $stock_status = isset($availability['class'])
             ? $availability['class']
             : false;
         if ($product && $product->exists()): ?>
				<tr id="yith-wcwl-row-<?php echo esc_attr(
        $item->get_product_id()
    ); ?>" data-row-id="<?php echo esc_attr($item->get_product_id()); ?>">
					<?php if ($show_cb): ?>
						<td class="product-checkbox">
							<input type="checkbox" value="yes" name="items[<?php echo esc_attr(
           $item->get_product_id()
       ); ?>][cb]" />
						</td>
					<?php endif; ?>
           ...
           get_permalink(
               apply_filters(
                   ...
                   $item->get_product_id()
							...
							<a href="<?php echo esc_url(
           add_query_arg('remove_from_wishlist', $item->get_product_id())
       ); ?>" class="remove remove_from_wishlist" title="<?php echo apply_filters(
    'yith_wcwl_remove_product_wishlist_message_title',
    __('Remove this product', 'yith-woocommerce-wishlist')
); ?>"></a>
						</div>
					</td>
    ...
    'woocommerce_in_cartproduct_obj_title',
    $product->get_name(),
    $product
); ?></a>
       ...
       ?>
					...
					<?php if ($show_quantity): ?>
						<td class="product-quantity">
							...
							<?php if (
           !$no_interactions &&
           $wishlist->current_user_can('update_quantity')
       ): ?>
								<input type="number" min="1" step="1" name="items[<?php echo esc_attr(
            $item->get_product_id()
        ); ?>][quantity]" value="<?php echo esc_attr(
    $item->get_quantity()
); ?>" />
							<?php else: ?>
					...
					<?php if ($show_stock_status): ?>
						<td class="product-stock-status">
							...
							<?php echo 'out-of-stock' === $stock_status
           ? '<span class="wishlist-out-of-stock">' .
               esc_html(
                   apply_filters(
                       'yith_wcwl_out_of_stock_label',
                       __('Out of stock', 'yith-woocommerce-wishlist')
                   )
               ) .
               '</span>'
           : '<span class="wishlist-in-stock text-v-dark">' .
               esc_html(
                   apply_filters(
                       'yith_wcwl_in_stock_label',
                       __('In Stock', 'yith-woocommerce-wishlist')
                   )
               ) .
               '</span>'; ?>
           ...
           echo '<span class="dateadded">' .
               esc_html(
                   sprintf(
                       __('Added on: %s', 'yith-woocommerce-wishlist'),
                       $item->get_date_added_formatted()
                   )
               ) .
               '</span>';
       endif; ?>
							...
							<?php do_action(
           'yith_wcwl_table_product_before_add_to_cart',
           $item,
           $wishlist
       ); ?>

							<!-- Add to cart button -->
							<?php $show_add_to_cart = apply_filters(
           'yith_wcwl_table_product_show_add_to_cart',
           $show_add_to_cart,
           $item,
           $wishlist
       ); ?>
           ...
           'out-of-stock' !== $stock_status
            ...
            'quantity' => $show_quantity ? $item->get_quantity() : 1,
							...
							<?php do_action(
           'yith_wcwl_table_product_after_add_to_cart',
           $item,
           $wishlist
       ); ?>

							<!-- Change wishlist -->
							<?php $move_to_another_wishlist = apply_filters(
           'yith_wcwl_table_product_move_to_another_wishlist',
              ...
              if ($wl->get_token() === $wishlist_token) {
                  continue;
											...
											<option value="<?php echo esc_attr($wl->get_token()); ?>">
												<?php echo sprintf(
                '%s - %s',
                esc_html($wl->get_formatted_name()),
                esc_html($wl->get_formatted_privacy())
            ); ?>
											</option>
											<?php
          endforeach; ?>
								...
								<?php else: ?>
									<a href="#move_to_another_wishlist" class="move-to-another-wishlist-button" data-rel="prettyPhoto[move_to_another_wishlist]">
										<?php echo esc_html(
              apply_filters(
                  'yith_wcwl_move_to_another_list_label',
                  __(
                      'Move to another list &rsaquo;',
                      'yith-woocommerce-wishlist'
                  )
              )
          ); ?>
								...
								<?php do_action(
            'yith_wcwl_table_product_after_move_to_another_wishlist',
            $item,
            $wishlist
        ); ?>
							...
							<?php if ($repeat_remove_button): ?>
								<a href="<?php echo method_exists('YITH_WCWL_Wishlist_Item', 'get_remove_url')
            ? esc_url($item->get_remove_url())
            : '#'; ?>" class="remove_from_wishlist button" title="<?php echo esc_attr(
    apply_filters(
        'yith_wcwl_remove_product_wishlist_message_title',
        __('Remove this product', 'yith-woocommerce-wishlist')
    )
); ?>"><?php esc_html_e('Remove', 'yith-woocommerce-wishlist'); ?></a>
							<?php endif; ?>
					...
					<?php if ($enable_drag_n_drop): ?>
						<td class="product-arrange ">
							...
							<input type="hidden" name="items[<?php echo esc_attr(
           $item->get_product_id()
       ); ?>][position]" value="<?php echo esc_attr(
    $item->get_position()
); ?>" />
						</td>
					<?php endif; ?>
     ...
     endforeach;
      ...
      ?>
       ...
       $column_count
   ); ?>" class="px-3 py-2 text-center"><i class="far fa-heart wishlist-empty"></i></td>
		</tr>
		<tr class="border-0 py-0">
			<td colspan="<?php echo esc_attr(
       $column_count
   ); ?>" class="px-3 py-2 wishlist-empty"><?php echo apply_filters(
    'yith_wcwl_no_product_to_remove_message',
    __('No products added to the wishlist', 'yith-woocommerce-wishlist')
); ?></td>
		</tr>
		<tr class="border-0 py-0">
			<td colspan="<?php echo esc_attr($column_count); ?>" class="px-3 text-center">
				<a class="woocommerce-Button button btn-v-dark btn-go-shop" href="<?php echo esc_url(
        apply_filters(
 ...
 if (!empty($page_links)): ?>
		<tr class="pagination-row">
			<td colspan="<?php echo esc_attr($column_count); ?>"><?php echo wp_kses_post(
    $page_links
); ?></td>
		</tr>
	<?php endif;
