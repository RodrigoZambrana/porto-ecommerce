...
if (defined('YITH_WOOCOMPARE')) {
    ...
    if (
        ...
        $yith_woocompare->obj instanceof YITH_Woocompare_Frontend
    ) {
        remove_action(
            'woocommerce_after_shop_loop_item',
        ...
        if (
            ...
            (!isset($_REQUEST['action']) ||
                'yith-woocompare-view-table' != $_REQUEST['action'])
        ) {
    ...
    if (is_admin()) {
        add_filter(
            'yith_woocompare_general_settings',
            'porto_check_compare_option'
        );
            ...
            if (!empty($compare_options['general'])) {
                foreach ($compare_options['general'] as $key => $option) {
                    if ('yith_woocompare_is_button' == $option['id']) {
                        $option['desc'] = sprintf(
                            esc_html__(
                                'Choose if you want to use a link or a button for the compare actions. %1$s %2$sThis option isn\'t available for %3$sPorto%4$s %5$s.',
                                'porto'
                            ),
                            '<br>',
                            '<span class="porto-warning-notice">',
                            '<a href="https://www.portotheme.com/wordpress/porto/" target="_blank"><b>',
                            '</b></a>',
                            '</span>'
                        );
                        ...
                        $option['desc'] = sprintf(
                            esc_html__(
                                'Type the text to use for the button or the link of the compare. %1$s %2$sThis option isn\'t available for %3$sPorto%4$s %5$s.',
                                'porto'
                            ),
                            '<br>',
                            '<span class="porto-warning-notice">',
...
add_action(
    ...
    'porto_woocommerce_open_before_clearfix_div',
    11
);
add_action(
    'woocommerce_before_shop_loop',
    'porto_woocommerce_close_before_clearfix_div',
    80
);
add_action(
    'woocommerce_after_shop_loop',
    'porto_woocommerce_open_after_clearfix_div',
    1
);
add_action(
    'woocommerce_after_shop_loop',
    'porto_woocommerce_close_after_clearfix_div',
    999
);
...
add_action(
    'woocommerce_before_shop_loop_item_title',
    'porto_loop_product_thumbnail',
    10
);
add_action(
    'woocommerce_archive_description',
    'porto_woocommerce_category_image',
    2
);
add_action(
    'woocommerce_shop_loop_item_title',
    'porto_woocommerce_shop_loop_item_title_open',
    1
);
add_action(
    'woocommerce_shop_loop_item_title',
    'porto_woocommerce_shop_loop_item_title_close',
    100
);
//add_action( 'woocommerce_single_product_summary', 'woocommerce_template_single_price', 25 );
add_action(
    'porto_woocommerce_shop_loop_start',
    'porto_woocommerce_shop_loop_item_init_layout'
);
add_action(
    'porto_woocommerce_shop_loop_end',
    'porto_woocommerce_shop_loop_item_reset_layout'
);
add_action(
    'porto_woocommerce_before_shop_loop_item_title',
    'porto_woocommerce_shop_loop_item_category'
);
add_action(
    'woocommerce_shop_loop_item_title',
    'porto_woocommerce_shop_loop_item_title'
);
...
add_action(
    'woocommerce_single_product_summary',
    'porto_woocommerce_sale_product_period',
    15
);
add_action(
    'woocommerce_before_shop_loop_item_title',
...
add_action(
    'porto_woocommerce_loop_links_on_image',
...
add_filter(
    ...
    'porto_woocommerce_get_sale_end_date',
    100,
    3
);
...
add_filter(
    'woocommerce_available_variation',
    'porto_woocommerce_available_variation',
    10,
    3
);
add_filter(
    'woocommerce_related_products_args',
    'porto_remove_related_products',
    10
);
add_filter(
    'woocommerce_add_to_cart_fragments',
    'porto_woocommerce_header_add_to_cart_fragment'
);
add_filter(
    'woocommerce_get_catalog_ordering_args',
    'porto_woocommerce_get_catalog_ordering_args',
    10,
    3
);
add_filter(
    'woocommerce_before_widget_product_review_list',
    'porto_woocommerce_before_widget_product_review_list'
);
add_filter(
    'wc_add_to_cart_message_html',
    'porto_add_to_cart_message_html',
    10,
    3
);
...
function porto_woocommerce_shop_loop_item_init_layout()
{
    ...
    if (
        (isset($porto_woocommerce_loop['widget']) &&
            $porto_woocommerce_loop['widget']) ||
        (isset($porto_woocommerce_loop['use_simple_layout']) &&
            $porto_woocommerce_loop['use_simple_layout']) ||
        in_array($woocommerce_loop['addlinks_pos'], [
            'outimage_aq_onimage',
            'outimage_aq_onimage2',
            'awq_onimage',
            'onimage2',
            'onimage3',
        ])
...
function porto_woocommerce_shop_loop_item_reset_layout()
{
    ...
    if (
        isset($porto_woocommerce_loop['reset_add_to_cart']) &&
        $porto_woocommerce_loop['reset_add_to_cart']
    ) {
        add_action(
            'woocommerce_after_shop_loop_item',
            'woocommerce_template_loop_add_to_cart'
...
function porto_woocommerce_shop_loop_item_category()
{
    global $product,
        $woocommerce_loop,
        $porto_settings,
        $porto_woocommerce_loop;
    $legacy_mode = apply_filters('porto_legacy_mode', true);
    $legacy_mode =
        ($legacy_mode && !empty($porto_settings['product-wishlist'])) ||
        !$legacy_mode;
    if (
        ...
        (!isset($porto_woocommerce_loop['use_simple_layout']) ||
            !$porto_woocommerce_loop['use_simple_layout']) &&
        ('outimage_aq_onimage' == $woocommerce_loop['addlinks_pos'] ||
            'outimage_aq_onimage2' == $woocommerce_loop['addlinks_pos'])
    ) {
    ...
    if (
        (!isset($porto_woocommerce_loop['use_simple_layout']) ||
            !$porto_woocommerce_loop['use_simple_layout']) &&
        (!isset($porto_settings['product-categories']) ||
            $porto_settings['product-categories'])
    ) {
...
function porto_woocommerce_share()
{
    ...
    $legacy_mode = apply_filters('porto_legacy_mode', true);
    $legacy_mode =
        ($legacy_mode && !empty($porto_settings['product-share'])) ||
        !$legacy_mode;
    if (
        $porto_settings['share-enable'] &&
        'no' !== $share &&
        ('yes' === $share || ('yes' !== $share && $legacy_mode))
    ) {
...
function porto_woocommerce_open_before_clearfix_div()
{
    ...
    if (
        (!isset($porto_shop_filter_layout) ||
            'horizontal2' != $porto_shop_filter_layout) &&
...
function porto_woocommerce_open_after_clearfix_div()
{
    ...
    $class_suffix = '';
    if (
        isset($porto_settings['product-infinite']) &&
        'infinite_scroll' == $porto_settings['product-infinite']
    ) {
        $class_suffix = ' d-none';
    } elseif (
        isset($porto_settings['product-infinite']) &&
        'load_more' == $porto_settings['product-infinite']
    ) {
...
function porto_grid_list_toggle(
    ...
    if (
        ...
        ((function_exists('wc_get_loop_prop') &&
            !wc_get_loop_prop('is_paginated')) ||
            !woocommerce_products_will_display() ||
            isset($porto_woocommerce_loop['view']))
    ) {
        return;
    }
    ...
    if (isset($_COOKIE['gridcookie'])) {
        ...
        isset($woocommerce_loop['category-view']) &&
    ...
    if (!$icon_grid) {
        if (
            'offcanvas' === $porto_shop_filter_layout ||
            'horizontal' === $porto_shop_filter_layout
        ) {
            $icon_grid = 'fas fa-th';
        } else {
    ...
    ?>
	<div class="gridlist-toggle<?php echo !$el_class
     ? ''
     : ' ' . esc_attr($el_class); ?>">
		<a href="#" id="grid" class="<?php echo esc_attr($icon_grid),
      'grid' == $view_mode ? ' active' : ''; ?>" title="<?php esc_attr_e(
    'Grid View',
    'porto'
); ?>"></a><a href="#" id="list" class="<?php echo esc_attr($icon_list),
    'list' == $view_mode ? ' active' : ''; ?>" title="<?php esc_attr_e(
    'List View',
    'porto'
); ?>"></a>
	</div>
...
function porto_woocommerce_available_variation(
    $variations,
    $product,
    $variation
) {
    if (has_post_thumbnail($variation->get_id())) {
        ...
        $image_thumb_link = wp_get_attachment_image_src(
            $attachment_id,
            'shop_thumbnail'
        );
        if ($image_thumb_link) {
            $variations = array_merge($variations, [
                'image_thumb' => $image_thumb_link[0],
            ]);
            $image_thumb_link = wp_get_attachment_image_src(
                $attachment_id,
                'shop_single'
...
function porto_layered_nav_link($link, $term = false)
{
    ...
    if (false === $term && (is_product_category() || is_product_tag())) {
        if (isset($_GET['product_cat']) || isset($_GET['product_tag'])) {
            ...
            $link_arr = explode(
                '?',
                remove_query_arg([$tax, 'source_id', 'source_tax'], $link)
            );
            $link = $currnet_page;
            if (2 === count($link_arr)) {
                $link .= '?' . $link_arr[1];
    ...
    $link = esc_url(str_replace(['&#038;', '#038;'], '&amp;', $link));
    return $link;
}
...
function porto_loop_product_thumbnail()
{
    ...
    $id = get_the_ID();
    if (
        isset($porto_woocommerce_loop['image_size']) &&
        $porto_woocommerce_loop['image_size']
    ) {
    ...
    if (!empty($gallery) && !empty($porto_settings['category-image-hover'])) {
        ...
        if ($show_hover_img) {
            ...
            $attachment_image = apply_filters(
                'porto_lazy_load_images',
                wp_get_attachment_image($first_image_id, $size, false, [
                    'class' => 'hover-image',
                ])
            );
        }
    ...
    $thumb_image = get_the_post_thumbnail($id, $size, ['class' => '']);
...
function porto_widget_product_thumbnail()
{
    ...
    echo '<div class="inner' . ($attachment_image ? ' img-effect' : '') . '">';
    // show images
    echo porto_filter_output(
        apply_filters('porto_lazy_load_images', $thumb_image)
    );
    echo porto_filter_output(
        apply_filters('porto_lazy_load_images', $attachment_image)
    );
    echo '</div>';
}
...
function porto_remove_related_products($args)
{
    global $porto_settings;
    if (
        isset($porto_settings['product-related']) &&
        !$porto_settings['product-related']
    ) {
        return [];
    }
    return $args;
...
function porto_woocommerce_header_add_to_cart_fragment($fragments)
{
    ...
    $_cart_total =
        ...
        '</span>';
    $cart_text = empty($porto_settings['minicart-text'])
        ? __('Cart', 'porto')
        : $porto_settings['minicart-text'];
    $fragments['#mini-cart .cart-subtotal'] =
    ...
    $fragments['#mini-cart .cart-items'] =
        '<span class="cart-items">' . ((int) $_cart_qty) . '</span>';
    /* translators: %s: Cart items */
    $fragments['#mini-cart .cart-items-text'] =
        '<span class="cart-items-text">' .
        sprintf(_n('%d item', '%d items', $_cart_qty, 'porto'), $_cart_qty) .
        '</span>';

    if (!empty($porto_settings['show-icon-menus-mobile'])) {
        $fragments['.porto-sticky-navbar .cart-items'] =
            $fragments['#mini-cart .cart-items'];
    }
    return $fragments;
}
...
if (wp_doing_ajax()) {
    ...
    add_action(
        'wp_ajax_nopriv_porto_cart_item_remove',
        'porto_cart_item_remove'
    );
    ...
    add_action(
        'wp_ajax_porto_refresh_cart_fragment',
        'porto_refresh_cart_fragment'
    );
    add_action(
        'wp_ajax_nopriv_porto_refresh_cart_fragment',
        'porto_refresh_cart_fragment'
    );
    ...
    add_action(
        'wp_ajax_porto_refresh_wishlist_count',
        'porto_refresh_wishlist_count'
    );
    add_action(
        'wp_ajax_nopriv_porto_refresh_wishlist_count',
        'porto_refresh_wishlist_count'
    );
  ...
  if (empty($wishlist_items)): ?>
		<p class="empty-msg"><?php esc_html_e(
	...
	<?php else: ?>
      ...
      if ($product) {
          ...
          $thumbnail = $product->get_image('product-thumbnail', []);
          ...
          if (empty($product_permalink)) {
              ...
              echo '<a aria-label="product" href="' .
                  esc_url($product_permalink) .
                  '">' .
                  porto_filter_output($thumbnail) .
                  '</a>';
          }
          echo '</div>';
          ...
          if (empty($product_permalink)) {
              ...
              echo '<a aria-label="product" href="' .
                  esc_url($product_permalink) .
                  '" class="text-v-dark">' .
                  porto_filter_output($product_name) .
                  '</a>';
          }
          echo '<span class="quantity">' .
              porto_filter_output($product_price) .
              '</span>';
          echo '<span class="remove_from_wishlist remove" data-product_id="' .
              absint($id) .
              '"></span>';
  ...
  ); ?>" class="btn btn-dark btn-modern btn-block btn-sm"><?php esc_html_e(
    'Go To Wishlist',
    'porto'
); ?></a>
		<?php endif;
  ...
  exit();
    ...
    if (
        isset($_POST['action']) &&
        'remove_from_wishlist' == $_POST['action'] &&
        isset($_POST['from']) &&
        'theme' == $_POST['from']
    ) {
...
add_action(
    ...
    'porto_woo_review_display_gravatar_wrap_start',
    9
);
add_action(
    'woocommerce_review_before',
    'porto_woo_review_display_gravatar_wrap_end',
    11
);
...
add_filter(
    ...
    'porto_woo_review_gravatar_size'
);
...
function porto_product_quickview()
{
    ...
    if (defined('YITH_WCWL')) {
        ...
        if (
            has_action('woocommerce_single_product_summary', [
                $wcwl_ins,
                'print_button',
            ])
        ) {
    ...
    ?>
	<div class="quickview-wrap quickview-wrap-<?php echo absint(
     $post->ID
 ); ?> single-product<?php echo ' product-type-' .
     ...
     if (
         !isset($_REQUEST['variation_flag']) ||
         !$_REQUEST['variation_flag'] ||
         'false' == $_REQUEST['variation_flag']
     ):
      ...
      $assets_path =
          esc_url(str_replace(['http:', 'https:'], '', WC()->plugin_url())) .
          '/assets/';
      $frontend_script_path = $assets_path . 'js/frontend/';
      ?>
						var wc_add_to_cart_variation_params = 
						<?php echo array2json(
          apply_filters('wc_add_to_cart_variation_params', [
              'wc_ajax_url' => WC_AJAX::get_endpoint('%%endpoint%%'),
              'i18n_no_matching_variations_text' => esc_js(
                  __(
                      'Sorry, no products matched your selection. Please choose a different combination.',
                      'woocommerce'
                  )
              ),
              'i18n_make_a_selection_text' => esc_js(
                  __(
                      'Please select some product options before adding this product to your cart.',
                      'woocommerce'
                  )
              ),
              'i18n_unavailable_text' => esc_js(
                  __(
                      ...
                      'woocommerce'
						...
						jQuery(document).ready(function($) {
							$.getScript('<?php echo porto_filter_output(
...
function porto_woocommerce_shop_loop_item_title_open()
{
    ...
    $more_target = '';
    if (
        isset($porto_settings['catalog-enable']) &&
        $porto_settings['catalog-enable']
    ) {
        if (
            $porto_settings['catalog-admin'] ||
            (!$porto_settings['catalog-admin'] &&
                !(current_user_can('administrator') && is_user_logged_in()))
        ) {
            if (!$porto_settings['catalog-cart']) {
                if (
                    $porto_settings['catalog-readmore'] &&
                    'all' === $porto_settings['catalog-readmore-archive']
                ) {
                    $link = get_post_meta(
                        get_the_id(),
                        'product_more_link',
                    ...
                    $more_target = $porto_settings['catalog-readmore-target']
                        ? 'target="' .
                            esc_attr(
                                $porto_settings['catalog-readmore-target']
                            ) .
                            '"'
                        : '';
                }
    ...
    ?>
	<a class="product-loop-title" <?php echo porto_filter_output(
     $more_target
		...
		<div class="product-<?php echo porto_filter_output($label); ?>">
			<a href="<?php echo esc_url(get_permalink($post)); ?>">
             ...
             echo '<img src="' .
                 ...
                 '" alt="Placeholder" width="' .
                 wc_get_image_size('shop_thumbnail_image_width')['width'] .
                 '" height="' .
                 wc_get_image_size('shop_thumbnail_image_height')['height'] .
                 '" />';
         } ?>
								</span>
							</span>
							<span class="product-details">
								<span class="product-title"><?php echo get_the_title($post)
            ? get_the_title($post)
            : $post->ID; ?></span>
							</span>
						</span>
...
function porto_woocommerce_init()
{
    ...
    if (
        isset($porto_settings['catalog-enable']) &&
        apply_filters('porto_legacy_mode', true) &&
        empty($porto_settings['product-short-desc'])
    ) {
        remove_action(
            'woocommerce_single_product_summary',
        ...
        if (
            ...
            if (!$porto_settings['catalog-price']) {
                ...
                add_filter(
                    ...
                    'porto_woocommerce_get_price_html_empty',
                    100,
                    2
                );
                add_filter(
                    'woocommerce_cart_item_price',
                    'porto_woocommerce_get_price_empty',
                    100,
                    3
                );
                add_filter(
            ...
            if (!$porto_settings['catalog-cart']) {
                ...
                remove_filter(
                    ...
                    'porto_woocommerce_display_quantity_input_on_shop_page',
                    10,
                    2
                );
            }
            if (!$porto_settings['catalog-review']) {
                add_filter(
                    'pre_option_woocommerce_enable_review_rating',
                    'porto_woocommerce_disable_rating'
                );
                add_filter(
                    'woocommerce_product_tabs',
                    'porto_woocommerce_remove_reviews_tab',
                    98
                );
    ...
    if (
        isset($porto_settings['product-tabs-pos']) &&
        'below' == $porto_settings['product-tabs-pos']
    ) {
    ...
    // add to cart sticky
    if (
        isset($porto_settings['product-sticky-addcart']) &&
        $porto_settings['product-sticky-addcart']
    ) {
        add_action(
            'woocommerce_single_product_summary',
            'porto_woocommerce_product_sticky_addcart',
            5
        );
    }
    ...
    if (!function_exists('wc_product_class')):
        function wc_product_class($arg = [])
        {
            return post_class($arg);
        }
    endif;
    if (!function_exists('wc_get_product_class')):
        function wc_get_product_class($arg = [])
        {
...
function porto_woocommerce_readmore_button()
{
    ...
    $more_target = !empty($porto_settings['catalog-readmore-target'])
        ? 'target="' .
            esc_attr($porto_settings['catalog-readmore-target']) .
            '"'
        : '';
    if (!$more_link) {
        $more_link = apply_filters('the_permalink', get_permalink());
	...
	<?php endif;
    ?>
			<a <?php echo porto_filter_output($more_target); ?> href="<?php echo esc_url(
     $more_link
 ); ?>" class="single_add_to_cart_button button readmore"><?php echo wp_kses_post(
    $porto_settings['catalog-readmore-label']
); ?></a>
...
add_filter(
    'pre_option_woocommerce_shop_page_display',
    'porto_shop_page_display_ajax'
);
function porto_shop_page_display_ajax($value)
{
    $params = ['count', 'orderby', 'min_price', 'max_price'];
...
add_filter(
    'get_woocommerce_term_metadata',
    'porto_woocommerce_term_metadata_ajax',
    10,
    4
);
function porto_woocommerce_term_metadata_ajax(
    $value,
    $object_id,
    $meta_key,
    $single
...
add_action(
    ...
    'porto_woocommerce_add_stock_html',
    10
);
    ...
    if ($product->is_type('simple')) {
        ...
        $availability_html = empty($availability['availability'])
            ...
            : '<span class="product-stock ' .
                ...
                '">' .
                esc_html__('Availability', 'porto') .
                ': <span class="stock">' .
                esc_html($availability['availability']) .
                '</span></span>';

        echo apply_filters(
            'porto_woocommerce_stock_html',
            $availability_html,
...
function porto_woocommerce_init_layout()
{
    ...
    if (
        ...
        (!empty($post->post_content) &&
            strstr($post->post_content, '[product_page'))
    ) {
        $porto_product_layout = get_post_meta(
            get_the_ID(),
            'product_layout',
            true
        );
        ...
        $porto_product_layout =
            !$porto_product_layout &&
            isset($porto_settings['product-single-content-layout'])
    ...
    if (
        ...
        $term = get_queried_object();
        if ($term && isset($term->taxonomy) && isset($term->term_id)) {
            $porto_shop_filter_layout = get_metadata(
                $term->taxonomy,
                $term->term_id,
                'filter_layout',
                true
        ...
        if (!$porto_shop_filter_layout) {
            $porto_shop_filter_layout =
                $porto_settings['product-archive-filter-layout'];
        }
    }
    ...
    if (
        ...
        if (
            isset($porto_settings['product-review']) &&
            !$porto_settings['product-review']
        ) {
            remove_action(
                'woocommerce_after_shop_loop_item_title',
                'woocommerce_template_loop_rating',
        ...
        if (
            isset($porto_settings['product-price']) &&
            !$porto_settings['product-price']
        ) {
            remove_action(
                'woocommerce_after_shop_loop_item_title',
                'woocommerce_template_loop_price',
    ...
    if ($porto_product_layout) {
        if ('extended' === $porto_product_layout) {
            ...
            add_action(
                'woocommerce_single_product_summary',
                'porto_woocommerce_template_single_custom_block',
                11
            );
            remove_action(
                'woocommerce_single_variation',
            ...
            add_action(
                'porto_woocommerce_before_single_product_summary',
                'woocommerce_template_single_title',
                5
            );
            add_action(
            ...
            add_action(
                'porto_woocommerce_single_product_summary2',
                'woocommerce_template_single_add_to_cart',
                10
            );
            add_action(
        ...
        if ('left_sidebar' === $porto_product_layout) {
            add_action(
                'woocommerce_after_single_product_summary',
                'porto_woocommerce_output_related_products',
                20
            );
        } elseif ('builder' !== $porto_product_layout) {
            add_action(
                'porto_after_content_bottom',
                'porto_woocommerce_output_related_products',
                8
            );
        }
        ...
        if (defined('YITH_WCWL')) {
            ...
            if (
                ...
                if (!porto_check_builder_condition('product')) {
                    if (
                        in_array($porto_product_layout, [
                            'extended',
                            'full_width',
                            'sticky_info',
                            'sticky_both_info',
                            'centered_vertical_zoom',
                        ])
                    ) {
                        add_action(
        ...
        /* compare position */
        if (
            defined('YITH_WOOCOMPARE') &&
            !porto_check_builder_condition('product')
        ) {
            if (
                'yes' ==
                get_option(
                    'yith_woocompare_compare_button_in_product_page',
                    'yes'
                )
            ) {
                if (
    ...
    if (
        isset($porto_settings['product-infinite']) &&
        $porto_settings['product-infinite']
    ) {
        remove_action(
            'woocommerce_before_shop_loop',
            'woocommerce_pagination',
    ...
    if (
        isset($porto_settings['product-show-price-role']) &&
        !empty($porto_settings['product-show-price-role'])
    ) {
        $hide_price = false;
        if (!is_user_logged_in()) {
            ...
            foreach (wp_get_current_user()->roles as $role => $val) {
                if (
                    !in_array($val, $porto_settings['product-show-price-role'])
                ) {
                    $hide_price = true;
                    break;
                }
...
function porto_woocommerce_template_single_custom_block()
{
    ...
    if ($block_slug) {
        ...
        echo do_shortcode(
            '[porto_block name="' .
                esc_attr($block_slug) .
                '" tracking="meta-product_custom_block"]'
        );
        echo '</div>';
    }
}
...
{
    global $porto_settings;
    $custom_tabs_count = isset($porto_settings['product-custom-tabs-count'])
        ? $porto_settings['product-custom-tabs-count']
        : '2';
    if ($custom_tabs_count) {
        for ($i = 0; $i < $custom_tabs_count; $i++) {
            $index = $i + 1;
            $custom_tab_title = get_post_meta(
                get_the_id(),
                'custom_tab_title' . $index,
                true
            );
            $custom_tab_priority = (int) get_post_meta(
                get_the_id(),
                'custom_tab_priority' . $index,
                true
            );
            if (!$custom_tab_priority) {
                $custom_tab_priority = 40 + $i;
            }
            $custom_tab_content = get_post_meta(
                get_the_id(),
                'custom_tab_content' . $index,
                true
            );
            if ($custom_tab_title && $custom_tab_content) {
                $tabs['custom_tab' . $index] = [
                    'title' => wp_kses_post($custom_tab_title),
                    'priority' => $custom_tab_priority,
                    'callback' => 'porto_woocommerce_custom_tab_content',
                    'content' => porto_output_tagged_content(
                        $custom_tab_content
                    ),
                ];
            }
...
function porto_woocommerce_global_tab($tabs)
{
    ...
    $custom_tab_content =
        '[porto_block name="' .
        $porto_settings['product-tab-block'] .
        '" tracking="option-product-tab-block"]';
    $custom_tab_priority =
        isset($porto_settings['product-tab-priority']) &&
        $porto_settings['product-tab-priority']
            ? $porto_settings['product-tab-priority']
            : 60;
    if ($custom_tab_title && $custom_tab_content) {
        $tabs['global_tab'] = [
...
function porto_wooc_extra_register_start_fields()
{
    global $porto_settings;
    if (
        isset($porto_settings['reg-form-info']) &&
        'full' == $porto_settings['reg-form-info']
    ): ?>
		<p class="form-row form-row-first">
			<label for="reg_billing_first_name"><?php esc_html_e(
       'First Name',
       'porto'
   ); ?><span class="required">*</span></label>
			<input type="text" class="woocommerce-Input woocommerce-Input--text input-text" name="billing_first_name" id="reg_billing_first_name" value="<?php echo !empty(
       $_POST['billing_first_name']
   )
       ? esc_attr($_POST['billing_first_name'])
       : ''; ?>" />
		</p>
		<p class="form-row form-row-last">
			<label for="reg_billing_last_name"><?php esc_html_e(
       'Last Name',
       'porto'
   ); ?><span class="required">*</span></label>
			<input type="text" class="woocommerce-Input woocommerce-Input--text input-text" name="billing_last_name" id="reg_billing_last_name" value="<?php echo !empty(
       $_POST['billing_last_name']
   )
       ? esc_attr($_POST['billing_last_name'])
       : ''; ?>" />
		</p>
		<div class="clear"></div>
		<?php endif;
...
add_action(
    ...
    'porto_wooc_extra_register_start_fields'
);
/**
 ...
 * @return void
 */
function porto_wooc_validate_extra_register_fields(
    $username,
    $email,
    $validation_errors
) {
    ...
    if (
        ...
        if (
            ...
            $validation_errors->add(
                ...
                __('<strong>Error</strong>: First name is required!', 'porto')
            );
        }
        if (
            ...
            $validation_errors->add(
                ...
                __('<strong>Error</strong>: Last name is required!.', 'porto')
            );
        }
    }
}
add_action(
    ...
    'porto_wooc_validate_extra_register_fields',
    10,
    3
);
/**
...
add_action(
    ...
    'porto_wooc_save_extra_register_fields'
);
...
function registration_errors_validation(
    $reg_errors,
    ...
    if (
        ...
        if (
            ...
            return new WP_Error(
                'registration-error',
                __('Passwords do not match.', 'porto')
            );
        }
        return $reg_errors;
    }
...
function porto_cart_version()
{
    global $porto_settings;
    $cart_ver =
        isset($porto_settings['cart-version']) &&
        $porto_settings['cart-version']
            ? $porto_settings['cart-version']
            : 'v1';
...
function porto_wc_body_class($classes)
{
            ...
            if (
                ...
                ((isset($_POST['product_id']) &&
                    ...
                    (isset($_POST['add-to-cart']) &&
                        $_POST['add-to-cart'] == $product->get_id()))
            ) {
                $classes[] = 'single-add-to-cart';
            }
        }
    ...
    global $porto_settings;
    if (
        !isset($porto_settings['woo-account-login-style']) ||
        !$porto_settings['woo-account-login-style']
    ) {
        $classes[] = 'login-popup';
    }
...
function porto_checkout_version()
{
    global $porto_settings;
    $checkout_ver =
        isset($porto_settings['checkout-version']) &&
        $porto_settings['checkout-version']
            ? $porto_settings['checkout-version']
            : 'v1';
...
function porto_wc_checkout_page()
{
    ...
    if (porto_checkout_version() == 'v2') {
        add_action(
            ...
            'porto_woocommerce_review_order_before_payment'
        );
    }
}
...
if (class_exists('WC_Vendors')) {
    add_action(
        'woocommerce_after_shop_loop_item_title',
        'porto_wpvendors_product_seller_name',
        2
    );
    remove_action(
        'woocommerce_product_meta_start',
    ...
    function porto_wpvendors_product_seller_name()
    {
        ...
        $vendor_display_name = WC_Vendors::$pv_options->get_option(
            'vendor_display_name'
        );
        ...
        switch ($vendor_display_name) {
            ...
            default:
                $vendor_name = '';
                $vendor_name = WCV_Vendors::is_vendor($author)
                    ? WCV_Vendors::get_vendor_shop_name($author)
                    : get_bloginfo('name');
        }
        ...
        $sold_by = WCV_Vendors::is_vendor($author)
            ? sprintf(
                '<a href="%s">%s</a>',
                WCV_Vendors::get_vendor_shop_page($author),
                $vendor_name
            )
            : get_bloginfo('name');
        if (!empty($porto_settings['porto_wcvendors_shop_soldby'])) {
            echo '<p class="product-seller-name">' .
                apply_filters('wcvendors_sold_by_in_loop', __('By', 'porto')) .
                ' <span>' .
                wp_kses_post($sold_by) .
                '</span> </p>';
        }
    }
    ...
    function porto_wc_vendors_sold_by_meta()
    {
        ...
        switch ($vendor_display_name) {
            ...
            default:
                $vendor_name = '';
                $vendor_name = WCV_Vendors::is_vendor($author_id)
                    ? WCV_Vendors::get_vendor_shop_name($author_id)
                    : get_bloginfo('name');
        }
        ...
        $sold_by = WCV_Vendors::is_vendor($author_id)
            ? sprintf(
                '<a href="%s">%s</a>',
                WCV_Vendors::get_vendor_shop_page($author_id),
                $vendor_name
            )
            : get_bloginfo('name');

        if (!empty($porto_settings['porto_wcvendors_product_soldby'])) {
            echo '<ul class="list-item-details"><li class="list-item-details"><span class="data-type">' .
                esc_html__('Sold by : ', 'porto') .
    ...
    function save_extra_user_profile_fields($user_id)
    {
        ...
        update_user_meta(
            ...
            'picture',
            $r,
            get_user_meta($user_id, 'picture', true)
        );
    }
    ...
    function porto_more_seller_product()
    {
        ...
        if ($products->have_posts()):
            ...
            global $porto_woocommerce_loop;
            $porto_woocommerce_loop['columns'] = isset(
                $porto_settings['product-related-cols']
            )
                ? $porto_settings['product-related-cols']
                : (isset($porto_settings['product-cols'])
                    ? $porto_settings['product-cols']
                    : 3);
			...
			<div class="related products">
				<h2 class="slider-title"><span class="inline-title"><?php esc_html_e(
        'More from this seller&hellip;',
        'porto'
    ); ?></span></h2>
...
add_filter(
    ...
    'porto_add_product_attribute_color',
    10,
    1
);
...
add_action(
    ...
    'porto_add_product_attribute_color_variation',
    10,
    2
);
...
{
    if (
        'color' !== $attribute_taxonomy->attribute_type &&
        'label' !== $attribute_taxonomy->attribute_type &&
        'image' !== $attribute_taxonomy->attribute_type
    ) {
        return;
    }
    ...
    if (
        ...
        $product_type = !empty($_POST['product_type'])
            ? wc_clean($_POST['product_type'])
            : 'simple';
    ...
    if ($product_object) {
        ...
        if (
            !array_key_exists(
                'pa_' . sanitize_title($attribute_taxonomy->attribute_name),
                $attributes
            )
        ) {
            return;
    ...
    ?>
	<select multiple="multiple" data-placeholder="<?php esc_attr_e(
     'Select terms',
     'woocommerce'
  ...
  if ($all_terms) {
      foreach ($all_terms as $term) {
          ...
          echo '<option value="' .
              ...
              '" ' .
              selected(in_array($term->term_id, $options), true, false) .
              '>' .
              esc_html(
                  apply_filters(
  ...
  ?>
	...
	<button class="button plus select_all_attributes"><?php esc_html_e(
     'Select all',
     'woocommerce'
 ); ?></button>
	<button class="button minus select_no_attributes"><?php esc_html_e(
     'Select none',
     'woocommerce'
 ); ?></button>
	<button class="button fr plus add_new_attribute"><?php esc_html_e(
     'Add new',
     'woocommerce'
 ); ?></button>
	<?php
...
add_filter(
    ...
    'porto_woocommerce_dropdown_variation_attribute_options_html',
    10,
    2
);
function porto_woocommerce_dropdown_variation_attribute_options_html(
    $select_html,
    $args
) {
    global $porto_settings;
    ...
    if (
        isset($porto_settings['product-attr-desc']) &&
        $porto_settings['product-attr-desc'] &&
        !empty($attribute) &&
        !empty($product) &&
        ...
        $attr_description_html .=
            ...
            ($args['selected'] ? ' active' : '') .
            '"><a href="#"><i class="fas fa-exclamation-circle"></i> ' .
            sprintf(
                esc_html__('Read More About %s', 'porto'),
                '<span>' .
                    ($args['name']
                        ? esc_html($args['name'])
                        : wc_attribute_label($attribute) . '</span>')
            ) .
            '</a><div>';
        foreach ($terms as $term) {
            if (in_array($term->slug, $options)) {
                if ($term->description) {
                    $attr_description_html .=
                        '<div class="attr-desc' .
                        (sanitize_title($args['selected']) == $term->slug
                            ? '  active'
                            : '') .
                        '" data-attrid="' .
                        esc_attr($term->slug) .
                    ...
                    $attr_description_html .=
                        ...
                        esc_attr($term->slug) .
                        '">' .
                        esc_html__('No description', 'porto') .
                        '</div>';
                }
            }
        }
    ...
    if (
        isset($porto_settings['product_variation_display_mode']) &&
        'select' === $porto_settings['product_variation_display_mode']
    ) {
    ...
    if ($attribute_taxonomies) {
        foreach ($attribute_taxonomies as $tax) {
            if (
                wc_attribute_taxonomy_name($tax->attribute_name) === $attribute
            ) {
    ...
    if (!empty($options)) {
        ...
        $html .=
            '<ul class="filter-item-list" data-name="' . esc_attr($name) . '">';
        if ($product) {
            $select_html =
                '<select id="' .
                esc_attr($id) .
            ...
            if (
                isset($swatch_options[$key]) &&
                isset($swatch_options[$key]['type'])
            ) {
                if (
                    'color' != $attr_type &&
                    'color' == $swatch_options[$key]['type']
                ) {
                    $attr_type = 'color';
            ...
            foreach ($attribute_terms as $term) {
                ...
                if (
                    (!isset($color_value) || !$color_value) &&
                    isset($swatch_options[$key]) &&
                    isset(
                        $swatch_options[$key]['attributes'][$term['id']][
                            'color'
                        ]
                    )
                ) {
                ...
                if ('image' == $attr_type) {
                    if (
                        ...
                        $current_attribute_image_id =
                            $swatch_options[$key]['attributes'][$term['id']][
                                'image'
                            ];
                    } else {
                        $current_attribute_image_id = get_term_meta(
                            $term['term_id'],
                            'image_value',
                            true
                        );
                    ...
                    if ($current_attribute_image_id) {
                        $current_attribute_image_src = wp_get_attachment_image_src(
                            $current_attribute_image_id,
                            $image_size
                        );
                        $current_attribute_image_src =
                            $current_attribute_image_src[0];
                    }
                }
                ...
                if ('color' == $attr_type) {
                    ...
                    $option_attrs =
                        ' data-image="' .
                        esc_url($current_attribute_image_src) .
                        '"';
                ...
                $selected =
                    sanitize_title($args['selected']) === $args['selected']
                        ? selected(
                            $args['selected'],
                            sanitize_title($term['slug']),
                            false
                        )
                        : selected($args['selected'], $term['slug'], false);
                $is_active =
                    sanitize_title($args['selected']) === $args['selected']
                        ? $args['selected'] == sanitize_title($term['slug'])
...
add_filter(
    ...
    'porto_woocommerce_layed_nav_color_html',
    10,
    4
);
function porto_woocommerce_layed_nav_color_html(
    $term_html,
    $term,
    $link,
    $count
    ...
    if ($color_value) {
        $attrs =
            ' class="filter-color" style="background-color: ' .
            esc_attr($color_value) .
            '"';
    }
    ...
    if (isset($_GET['product_cat'])) {
        $link = add_query_arg(
            ...
            remove_query_arg('product_cat', $link)
        );
    }
    ...
    if ($color_value) {
        ...
        $before_html = str_replace(
            '"woocommerce-widget-layered-nav-list"',
            '"woocommerce-widget-layered-nav-list filter-item-list"',
            $before_html
        );
        ob_start();
...
add_filter(
    'yith_woocommerce_reset_filter_link',
    'porto_yith_woocommerce_reset_filter_link'
);
...
// add to wishlist
if (defined('YITH_WCWL')) {
    add_action('yith_wcwl_wishlist_after_wishlist_content', function ($var) {
        if (!empty($var['count'])) {
            global $porto_settings;
            $legacy_mode = apply_filters('porto_legacy_mode', true);
            $legacy_mode =
                ($legacy_mode &&
                    !empty($porto_settings['product-quickview'])) ||
                !$legacy_mode;
    ...
    add_filter(
        ...
        'porto_wishlist_page_share_option',
        20
    );
    ...
    function porto_yith_wcwl_template_part_hierarchy(
        $arr,
        $template,
        $template_part,
        $template_layout,
    ...
    if (is_admin()) {
        ...
        function porto_check_wishlist_option($wishlist_options)
        {
            if (!empty($wishlist_options['add_to_wishlist'])) {
                $wishlist_options['add_to_wishlist']['shop_page_section_start'][
                    'desc'
                ] = sprintf(
                    esc_html__(
                        'Loop options will be visible on Shop page, category pages, product shortcodes, products sliders, and all the other places where the WooCommerce products\' loop is used.%1$s %2$s %8$s%4$sPorto%5$s%9$s offers %8$s%6$sTheme Option%7$s/Product Archives/Show Wishlist%9$s. You should use that instead of this option.%3$s',
                        'porto'
                    ),
                    '<br>',
                    '<div class="porto-important-note yith-wishlist">',
                    '</div>',
                    '<a href="https://www.portotheme.com/wordpress/porto/" target="_blank">',
                    '</a>',
                    '<a href="' .
                        esc_url(admin_url('themes.php?page=porto_settings')) .
                        '" target="_blank">',
                    '</a>',
                    '<b>',
                    '</b>'
                ...
                $wishlist_options['add_to_wishlist']['add_to_wishlist_icon'][
                    ...
                    esc_html__(
                        'Select an icon for the "Add to wishlist" button (optional). %1$s %2$sThis option isn\'t available for %3$sPorto%4$s %5$s.',
                        'porto'
                    ),
                    '<br>',
                    '<span class="porto-warning-notice">',
                ...
                $wishlist_options['add_to_wishlist']['added_to_wishlist_icon'][
                    'desc'
                ] = sprintf(
                    esc_html__(
                        'Select an icon for the "Added to wishlist" button (optional). %1$s %2$sThis option isn\'t available for %3$sPorto%4$s %5$s.',
                        'porto'
                    ),
                    '<br>',
                    '<span class="porto-warning-notice">',
    ...
    function porto_wishlist_page_share_option($share_options)
    {
        if (!empty($share_options['color_share_button'])) {
            $share_options['color_share_button']['selector'] =
...
function porto_woocommerce_output_horizontal_filter()
{
    ...
    if (
        ...
        empty($porto_shop_filter_layout)
    ) {
        $porto_shop_filter_layout =
            $porto_settings['product-archive-filter-layout'];
    }
    ...
    if ('horizontal' === $porto_shop_filter_layout) {
        if (
            porto_is_ajax() &&
            isset($_COOKIE['porto_horizontal_filter']) &&
            'opened' == $_COOKIE['porto_horizontal_filter']
        ) {
            $class = ' opened';
        } else {
            $class = '';
        }
        echo '<span class="porto-product-filters-toggle d-none d-lg-flex' .
            $class .
            '"><span>' .
            esc_html__('Filters:', 'porto') .
            '</span><a href="#">&nbsp;</a></span>';
    ...
    if ('offcanvas' === $porto_shop_filter_layout) {
        echo '<a href="#" class="porto-product-filters-toggle sidebar-toggle d-inline-flex"><i class="fas fa-sliders-h"></i> <span>' .
            esc_html__('Filtrar', 'porto') .
            '</span></a>';
        $GLOBALS['porto_mobile_toggle'] = false;
    } elseif (
        !empty($porto_settings['show-mobile-sidebar']) ||
        'horizontal2' === $porto_shop_filter_layout
    ) {
        echo '<a href="#" class="porto-product-filters-toggle sidebar-toggle d-inline-flex d-lg-none"><svg data-name="Layer 3" id="Layer_3" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg"><line class="cls-1" x1="15" x2="26" y1="9" y2="9"/><line class="cls-1" x1="6" x2="9" y1="9" y2="9"/><line class="cls-1" x1="23" x2="26" y1="16" y2="16"/><line class="cls-1" x1="6" x2="17" y1="16" y2="16"/><line class="cls-1" x1="17" x2="26" y1="23" y2="23"/><line class="cls-1" x1="6" x2="11" y1="23" y2="23"/><path class="cls-2" d="M14.5,8.92A2.6,2.6,0,0,1,12,11.5,2.6,2.6,0,0,1,9.5,8.92a2.5,2.5,0,0,1,5,0Z"/><path class="cls-2" d="M22.5,15.92a2.5,2.5,0,1,1-5,0,2.5,2.5,0,0,1,5,0Z"/><path class="cls-3" d="M21,16a1,1,0,1,1-2,0,1,1,0,0,1,2,0Z"/><path class="cls-2" d="M16.5,22.92A2.6,2.6,0,0,1,14,25.5a2.6,2.6,0,0,1-2.5-2.58,2.5,2.5,0,0,1,5,0Z"/></svg> <span>' .
            esc_html__('Filter', 'porto') .
            '</span></a>';
        $GLOBALS['porto_mobile_toggle'] = false;
    }
...
function porto_woocommerce_sale_product_period($dynamic_product = false)
{
    ...
    if (
        ((!isset($porto_woocommerce_loop['widget']) ||
            ...
            $product &&
            $product->is_on_sale()) ||
        ($dynamic_product && $dynamic_product->is_on_sale())
    ) {
        $product_temp;
        if ($dynamic_product) {
        ...
        $is_single =
            (porto_is_product() &&
                ...
                !isset($GLOBALS['porto_woocommerce_loop'])) ||
            (porto_is_ajax() &&
                isset($_REQUEST['action']) &&
                'porto_product_quickview' == $_REQUEST['action']);
        $extra_class = '';
        ...
        if ($date_diff) {
            echo '<div class="sale-product-daily-deal' .
                $extra_class .
                '"' .
                ($extra_class ? ' style="display: none"' : '') .
                '>';
            echo '<h5 class="daily-deal-title">' .
                esc_html__('Offer Ends In:', 'porto') .
                '</h5>';
            if ($is_single) {
                echo do_shortcode(
                    '[porto_countdown datetime="' .
                        $date_diff .
                        '" countdown_opts="sday,shr,smin,ssec" string_days="' .
                        esc_attr__('Day', 'porto') .
                        '" string_days2="' .
                        esc_attr__('Days', 'porto') .
                        '" string_hours="' .
                        esc_attr__('Hour', 'porto') .
                        '" string_hours2="' .
                        esc_attr__('Hours', 'porto') .
                        '" string_minutes="' .
                        esc_attr__('Minute', 'porto') .
                        '" string_minutes2="' .
                        esc_attr__('Minutes', 'porto') .
                        '" string_seconds="' .
                        esc_attr__('Second', 'porto') .
                        '" string_seconds2="' .
                        esc_attr__('Seconds', 'porto') .
                        '"]'
                );
            } else {
                echo do_shortcode(
                    '[porto_countdown datetime="' .
                        ...
                        '" string_days2="' .
                        esc_attr__('Days', 'porto') .
                        '" string_hours=":" string_hours2=":" string_minutes=":" string_minutes2=":" string_seconds="" string_seconds2=""]'
                );
            }
            echo '</div>';
        }
...
add_action(
    'wp_ajax_nopriv_porto_account_login_popup',
    'porto_account_login_popup'
);
function porto_account_login_popup()
{
    ...
    if (
        ...
        if (
            ($is_facebook_login || $is_google_login || $is_twitter_login) &&
...
add_action(
    'wp_ajax_porto_account_login_popup_login',
    'porto_account_login_popup_login'
);
add_action(
    'wp_ajax_nopriv_porto_account_login_popup_login',
    'porto_account_login_popup_login'
);
function porto_account_login_popup_login()
{
        ...
        try {
            ...
            if (empty($creds['user_login'])) {
                echo json_encode([
                    ...
                    'message' =>
                        ...
                        esc_html__('Error:', 'woocommerce') .
                        '</strong> ' .
                        esc_html__('Username is required.', 'woocommerce'),
                ]);
                die();
            }
            ...
            if (is_multisite()) {
                $user_data = get_user_by(
                    is_email($creds['user_login']) ? 'email' : 'login',
                    $creds['user_login']
                );
            ...
            // Perform the login
            $user = wp_signon(
                apply_filters('woocommerce_login_credentials', $creds),
                is_ssl()
            );
            if (!is_wp_error($user)) {
                $result = true;
    ...
    if ($result) {
        ...
        echo json_encode([
            'loggedin' => false,
            'message' => esc_html__('Wrong username or password.', 'porto'),
        ]);
    }
    die();
}
add_action(
    'wp_ajax_porto_account_login_popup_register',
    'porto_account_login_popup_register'
);
add_action(
    'wp_ajax_nopriv_porto_account_login_popup_register',
    'porto_account_login_popup_register'
);
function porto_account_login_popup_register()
{
    $nonce_value = isset($_POST['_wpnonce']) ? $_POST['_wpnonce'] : '';
    $nonce_value = isset($_POST['woocommerce-register-nonce'])
    ...
    if ($result) {
        ...
        echo json_encode([
            'loggedin' => false,
            'message' => esc_html__('Register failed.', 'porto'),
        ]);
    }
    die();
}
...
add_action(
    'wp_ajax_porto_woocommerce_shortcodes_products',
    'porto_woocommerce_shortcodes_products'
);
add_action(
    'wp_ajax_nopriv_porto_woocommerce_shortcodes_products',
    'porto_woocommerce_shortcodes_products'
);
function porto_woocommerce_shortcodes_products()
{
    ...
    if (isset($_POST['autoplay_timeout'])) {
        $atts .=
            ' autoplay_timeout="' . esc_attr($_POST['autoplay_timeout']) . '"';
    }
...
add_action(
    'woocommerce_before_subcategory_title',
    'porto_woocommerce_subcategory_thumbnail',
    10
);
...
add_action(
    'woocommerce_after_shop_loop_item',
    'porto_woocommerce_display_variation_on_shop_page'
);
...
function porto_woocommerce_display_quantity_input_on_shop_page($html, $product)
{
    ...
    $availability = $product->get_availability();
    $stock_status = !empty($availability['class'])
        ? $availability['class']
        : '';
    if (
        ((isset($porto_settings['category-addlinks-pos']) &&
            'quantity' == $porto_settings['category-addlinks-pos']) ||
            (isset($porto_woocommerce_loop['addlinks_pos']) &&
                'quantity' == $porto_woocommerce_loop['addlinks_pos'])) &&
...
function porto_woocommerce_display_variation_on_shop_page($el_class = '')
{
    ...
    if (
        (!isset($porto_woocommerce_loop['widget']) ||
            !$porto_woocommerce_loop['widget']) &&
        isset($porto_settings['show_swatch']) &&
        $porto_settings['show_swatch'] &&
        $product->is_type('variable')
    ) {
        $attributes = $product->get_variation_attributes();
        $woocommerce_taxonomies = wc_get_attribute_taxonomies();
        $woocommerce_taxonomy_infos = [];
        ...
        foreach ($attributes as $key => $attr) {
            if (
                array_key_exists($key, $woocommerce_taxonomy_infos) &&
                isset($woocommerce_taxonomy_infos[$key]->attribute_type)
            ) {
                if (
                    'color' != $woocommerce_taxonomy_infos[$key]->attribute_type
                ) {
                    if ($swatch_options) {
                        $swatch_key = md5(sanitize_title($key));
                        if (
                            !isset($swatch_options[$swatch_key]) ||
                            !isset($swatch_options[$swatch_key]['type']) ||
                            ('color' != $swatch_options[$swatch_key]['type'] &&
                                'image' != $swatch_options[$swatch_key]['type'])
                        ) {
...
if (!function_exists('porto_woocommerce_product_sticky_addcart')):
    ...
    function porto_woocommerce_product_sticky_addcart($el_class = '')
    {
        ...
        global $product;
        $attachment_id = method_exists($product, 'get_image_id')
            ? $product->get_image_id()
            : get_post_thumbnail_id();
        $availability = $product->get_availability();
        $average = $product->get_average_rating();

        echo '<div class="sticky-product hide pos-' .
            esc_attr($porto_settings['product-sticky-addcart']) .
        ...
        echo '</div>';
        echo '<div class="availability"><span>' .
            ('out-of-stock' == $availability['class']
                ? esc_html__('Out of stock', 'porto')
                : esc_html__('In stock', 'porto')) .
            '</span></div>';
        echo '</div>';
...
if (!function_exists('porto_woocommerce_add_to_cart_notification_html')):
    ...
    {
        global $porto_settings; ?>
		<div class="after-loading-success-message style-<?php echo esc_attr(
      $porto_settings['add-to-cart-notification']
  ); ?>">
		<?php if (
      isset($porto_settings['add-to-cart-notification']) &&
      2 === (int) $porto_settings['add-to-cart-notification']
  ): ?>
			<div class="background-overlay"></div>
			<div class="loader success-message-container">
				<div class="msg-box">
					<div class="msg"><?php esc_html_e(
         'Producto agregado al carrito',
         'porto'
     ); ?>:<p class="product-name text-color-primary"></p></div>
				</div>
				<button class="button btn-primay viewcart" data-link=""><?php esc_html_e(
        'Ver carrito',
        'porto'
    ); ?></button>
				<button class="button btn-primay continue_shopping"><?php esc_html_e(
        'Ahora no',
        'porto'
    ); ?></button>
			</div>
	<?php
      /* translators: product name div */
      ?>else: ?>
      ...
      ?>
						<?php printf(
          esc_html__('%s ha sido agregado al carrito.', 'porto'),
          '<div class="product-name"></div>'
      ); ?>
					</div>
				</div>
				<button class="btn btn-modern btn-sm btn-gray viewcart btn-sm" data-link=""><?php esc_html_e(
        'View Cart',
        'porto'
    ); ?></button>
				<a class="btn btn-modern btn-sm btn-dark continue_shopping" href="<?php echo esc_url(
        function_exists('wc_get_checkout_url')
            ? wc_get_checkout_url()
            : wc_get_page_permalink('checkout')
    ); ?>"><?php esc_html_e('Finalizar compra', 'porto'); ?></a>
...
add_action(
    'woocommerce_after_subcategory',
    'porto_product_categories_add_featured',
    20,
    1
);
if (!function_exists('porto_product_categories_add_featured')):
    function porto_product_categories_add_featured($category)
    {
        ...
        if (
            isset(
                $porto_woocommerce_loop['product_categories_show_featured']
            ) &&
            $porto_woocommerce_loop['product_categories_show_featured'] &&
            $category
        ) {
            ...
            echo do_shortcode(
                '[featured_products per_page="1" columns="1" category="' .
...
if (!function_exists('porto_woocommerce_get_catalog_ordering_args')):
     ...
     * Add sales order by
     */
    function porto_woocommerce_get_catalog_ordering_args(
        $args,
        $orderby,
        $order
    ) {
...
function porto_add_to_cart_message_html($message, $products, $show_qty)
{
    ...
    $added_text = sprintf(
        _n(
            '<strong class="single-cart-notice">%s</strong> <span class="font-weight-medium">ha sido agregado al carrito.</span>',
            '<strong class="single-cart-notice">%s</strong> <span class="font-weight-medium">ha sido agregado al carrito.</span>',
            $count,
            'porto'
        ),
    ...
    if ('yes' === get_option('woocommerce_cart_redirect_after_add')) {
        ...
        $message = sprintf(
            ...
            esc_url($return_to),
            esc_html__('Ahora no', 'woocommerce'),
            $added_text
        );
    } else {
...
add_filter(
    ...
    function ($has_passed) {
        if ($has_passed) {
            add_action(
                'woocommerce_after_add_to_cart_button',
                'porto_view_cart_after_add',
                defined('WC_STRIPE_PLUGIN_NAME') ? 8 : 35
            );
        }
        return $has_passed;
    },
...
if (!function_exists('porto_template_loop_compare')) {
    function porto_template_loop_compare(
        $extra_class = '',
        $hide_title = false,
        $icon_cls = '',
        $icon_pos = 'left',
        ...
        if (defined('YITH_WOOCOMPARE') && isset($product)) {
            $css_class =
                ...
                ($extra_class ? ' ' . trim($extra_class) : ' button');
            $product_id = $product->get_id();
            $url_args = ['id' => $product_id];
            ...
            $the_list = isset($_COOKIE[$cookie_name])
            ...
            if (in_array($product_id, $the_list)) {
                ...
                if (!$hide_title) {
                    $button_text = esc_html(
                        apply_filters(
                            'yith_woocompare_compare_added_label',
                            __('Already Added', 'porto')
                        )
                    );
                }
            ...
            printf(
                '<a href="%s" class="%s" title="%s" data-product_id="%d"' .
                    ($icon_cls_added
                        ? ' data-added_icon="' .
                            esc_attr($icon_cls_added) .
                            '"' .
...
if (!function_exists('porto_update_cart_item')) {
    ...
    add_action(
        'wp_ajax_nopriv_porto_update_cart_item',
        'porto_update_cart_item'
    );
}
